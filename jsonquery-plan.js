...
// @options = {sort:{String: 1|-1}, limit: Integer}
function optimizer (query, options, plan) {
  streamOptions = {
    limit: -1 ,
    reverse: false
  }
  if('object' == typeof options){
    if(Object.hasOwnProperty.call(options, 'limit'))
      ...
      streamOptions.limit = -1
    if(Object.hasOwnProperty.call(options, 'reverse'))
      streamOptions.reverse = options.reverse;
    else
      streamOptions.reverse = false;
  }
...
function optimizerMatch (predicate, negate, plan) {
  ...
  for (var n in predicate) {
    ...
    if (v === undefined) {
      ...
      idxStream = plan(n, '$eq', v, negate, streamOptions);
      if (idxStream) sources.push(idxStream);
    }
  }
...
function optimizeValOp (op, val, args, negate, plan) {
  ...
  switch (op) {
    case '$in':
      ...
      if (Array.isArray(args)) {
        for (i = 0; i < args.length; i++) {
          arg = args[i];
          idxStream = plan(val, '$eq', arg, negate, streamOptions);
          if (idxStream) sources.push(idxStream);
        }
      }
      if (sources.length === 0) return null;
    ...
    case '$nin':
      return plan(val, args, args, !negate, streamOptions);
    ...
    case '$mod':
    case '$exists':
      return plan(val, op, args, negate, streamOptions);
